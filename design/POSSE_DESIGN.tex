...
\begin{itemize}[leftmargin=1.1em,itemsep=1.5pt,topsep=2pt]
  \item \textbf{Identity is tied to a live process.} In most systems a ``thread'' is
  a running object (a process, a graph run, a chat); when it ends, its identity and
  live context end with it (fails R2). Server-side threads~\cite{assistants} and
  durable-execution engines~\cite{temporal} are the exceptions, but for conversations
  and generic workflows, not agent fleets with cost and memory concerns.
  \item \textbf{No asynchronous interruption.} Graph frameworks do persist an
  \code{interrupt} and resume it later~\cite{langgraph}, and durable-execution engines
  deliver asynchronous \emph{signals} into a suspended workflow~\cite{temporal} --- the
  closest mechanisms to what we need. But neither models a human absent for hours who
  injects feedback into an \emph{LLM-agent} thread that has meanwhile been killed and
  replaced by a fresh process: Posse's interrupt kills and relaunches the agent process
  with a reply-first drain, where a workflow engine replays deterministically and runs a
  signal handler (fails R4 for the agent case).
  \item \textbf{No self-recovery over long horizons.} Frameworks retry a tool call;
  none, as a system, detect that an agent died overnight or hit a weekly rate limit
  and relaunch it after the reset (fails R1, R5).
  \item \textbf{Cost is unmanaged.} Observability prices runs \emph{after} the fact;
  no framework we surveyed \emph{schedules} around the provider's caching economics,
  where the gap between two touches of a context decides whether you pay
  $1\times$ or $\sim$12.5$\times$ (fails R6).
  \item \textbf{Memory is trapped in a session.} In the common pattern, a single
  agent accrues knowledge over a long session, so a \emph{different} agent cannot reuse
  it and sharing is ad hoc; keeping that one agent alive for the whole horizon is
  exactly the unbounded-context cost of R6. Standing-agent designs grow one context past
  the window; per-episode designs forget at the end. AIOS/MemGPT address the
  \emph{mechanism} of bounded memory but not the \emph{operational} question of a
  separate, zero-cost manager compacting durable per-domain records that \emph{any} agent
  can read (partially meets R7).
\end{itemize}
